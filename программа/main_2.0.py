...
class Model:
    ...
    def finder_extra(self):
        ...
        for line in self.quartet_line:
            for q in line:
                if self.check_frustration(q, self.field_change) == 0:
                    x, y = self.find_x_y(q[1])
                    self.spin(self.field_change, x, y)
                    self.steps += 1
...
class Interface:
    ...
    def draw_text(self):
        ...
        text3 = f1.render(f'Steps - {model.steps}', True, (50, 50, 50))

        self.screen.blit(text1, (x, y + 30))
        self.screen.blit(text2, (x + 400, y + 30))
        self.screen.blit(text3, (x + 800, y + 30))
